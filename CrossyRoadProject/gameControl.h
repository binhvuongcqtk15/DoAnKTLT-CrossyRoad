...
void drawCharacter(const POINT& p, char s) {
	GotoXY(0, 0);
	GotoXY(p.x, p.y);
	cout << s;
}
...
void moveCars() {
	for (int i = 1; i < MAX_CAR; i += 2) {
		...
		do {
			...
			carArray[i][MAX_CAR_LENGTH - 1].x + 1 == WIDTH_CONSOLE ? 
					carArray[i][MAX_CAR_LENGTH - 1].x = 1 : carArray[i][MAX_CAR_LENGTH - 1].x++;
		} while (cnt < speed);
	}
	for (int i = 1; i < MAX_CAR; i += 2) {
		cnt = 0;
		do {
			...
			carArray[i][0].x - 1 == 0 ? 
					carArray[i][0].x = WIDTH_CONSOLE - 1 : carArray[i][0].x--;
		} while (cnt < speed);
	}
}
